...
const Content = () => {
  ...
  const playlists = Object.keys(state.playlists).filter(
    list => !['Inicio', 'Favoritos'].includes(list)
  )
    ...
    <div className="Content" css={CSS}>
      ...
      {songIds.length <= 0 ? (
        <p style={{ marginTop: 10 }}>
          Sua playlist está vazia. Comece adicionando algumas músicas!
        </p>
      ) : (
        <table>
          <thead>
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          {playlists.length < 1 ? (
            <>
              <p>
                Você ainda não tem playlists personalizadas.
                Comece criando uma no menu da barra lateral!
              </p>
              ...
              <div style={{ marginTop: 20 }}>
                <button
                  onClick={() => {
                    ...
                    setToast(
                      'Adicionado a sua Playlist com sucesso!'
                    )
